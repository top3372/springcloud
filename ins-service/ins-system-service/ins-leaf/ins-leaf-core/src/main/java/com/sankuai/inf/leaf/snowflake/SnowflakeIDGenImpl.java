...
import com.sankuai.inf.leaf.common.LeafConstant;
        ...
        if (timestamp < lastTimestamp) {
            ...
            if (offset <= 5) {
                try {
                    ...
                    timestamp = timeGen();
                    if (timestamp < lastTimestamp) {
                        return new Result(LeafConstant.EXCEPTION_ID_IDCACHE_INIT_FALSE, Status.EXCEPTION);
                    }
                } catch (InterruptedException e) {
                    LOGGER.error("wait interrupted");
                    return new Result(LeafConstant.EXCEPTION_ID_KEY_NOT_EXISTS, Status.EXCEPTION);
                }
            } else {
                return new Result(LeafConstant.EXCEPTION_ID_TWO_SEGMENTS_ARE_NULL, Status.EXCEPTION);
            }
        }
